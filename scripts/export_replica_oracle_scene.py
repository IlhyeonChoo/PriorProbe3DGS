...
from priorprobe.features import FeatureExtractorConfig
from priorprobe.replica_export import export_replica_oracle_scene
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export a Replica scene to COLMAP layout with oracle target metadata.")
    ...
    parser.add_argument("--output-root", default=Path("data/public_datasets/replica_colmap"), type=Path)
    ...
    scene_root = export_replica_oracle_scene(
        ...
        feature_config=FeatureExtractorConfig(render_count=8, allow_fallback=True),
