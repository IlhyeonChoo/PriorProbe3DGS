...
from priorprobe.shapesplat_modelnet import prepare_modelnet_prior_bundle
...
def parse_category_zip(values: list[str]) -> dict[str, Path]:
    payload: dict[str, Path] = {}
    for value in values:
        if "=" not in value:
            raise ValueError(f"--category-zip must be category=/path/to/zip, got: {value}")
        category, raw_path = value.split("=", 1)
        payload[category.strip()] = resolve_path(Path(raw_path.strip()))
    return payload


def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare a staged multi-category ShapeSplat ModelNet prior bundle.")
    parser.add_argument("--category-zip", action="append", required=True, help="category=/path/to/modelnet_zip")
    ...
    parser.add_argument("--max-objects-per-category", type=int, default=128)
    ...
    payload = prepare_modelnet_prior_bundle(
        category_zips=parse_category_zip(args.category_zip),
