...
def main() -> int:
    parser = argparse.ArgumentParser(description="Report Phase 5 surface RGB gaussian-direct runs.")
    ...
    for experiment in EXPERIMENTS:
        for scene_id in SCENES:
            ...
            if experiment["label"] == "baseline":
                ...
                baseline_15000 = checkpoint_by_iteration(checkpoints, 15000)
                ...
                if baseline_15000 is not None and baseline_15000.get("psnr") is not None:
                    baseline_final_psnr[scene_id] = float(baseline_15000["psnr"])
            ...
            protection_0 = load_prior_protection(model_path, 0)
            ...
            protection_15000_index = protection_group_index(protection_15000)
            ...
            prior_survived_15000 = sum(
                int(item.get("survived_point_count", 0) or 0) for item in protection_15000_index.values()
            )
            ...
            summary_rows.append(
                {
                    ...
                    "iter_0_ssim": iter_0.get("ssim"),
                    "iter_0_lpips": iter_0.get("lpips"),
                    "iter_0_gaussians": iter_0.get("gaussian_count"),
                    "iter_3000_psnr": iter_3000.get("psnr"),
                    "iter_3000_ssim": iter_3000.get("ssim"),
                    "iter_3000_lpips": iter_3000.get("lpips"),
                    "iter_3000_gaussians": iter_3000.get("gaussian_count"),
                    "iter_15000_psnr": iter_15000.get("psnr"),
                    "iter_15000_ssim": iter_15000.get("ssim"),
                    "iter_15000_lpips": iter_15000.get("lpips"),
                    "iter_15000_gaussians": iter_15000.get("gaussian_count"),
                    "delta_iter_0_psnr_vs_baseline": (
                        float(iter_0.get("psnr")) - baseline_target_psnr.get(scene_id, float("nan"))
                        if experiment["label"] != "baseline"
                        and iter_0.get("psnr") is not None
                        and scene_id in baseline_target_psnr
                        else None
                    ),
                    "delta_final_psnr_vs_baseline": (
                        float(iter_15000.get("psnr")) - baseline_final_psnr.get(scene_id, float("nan"))
                        if experiment["label"] != "baseline"
                        and iter_15000.get("psnr") is not None
                    ...
                    "prior_survived_15000": prior_survived_15000 if selected_priors else None,
                    "prior_survival_ratio_3000": (
                        float(prior_survived_3000 / prior_inserted_total) if prior_inserted_total > 0 else None
                    ),
                    "prior_survival_ratio_15000": (
                        float(prior_survived_15000 / prior_inserted_total) if prior_inserted_total > 0 else None
                    ),
                }
            )
            ...
            for item in selected_priors:
                ...
                g15000 = protection_15000_index.get(prior_object_id, {})
                ...
                object_rows.append(
                    {
                        ...
                        "survived_15000": g15000.get("survived_point_count"),
                        "survival_ratio_15000": g15000.get("survival_ratio"),
                    }
                )
    ...
    write_csv(summary_csv, list(summary_rows[0].keys()) if summary_rows else [], summary_rows)
    write_csv(checkpoints_csv, list(checkpoint_rows[0].keys()) if checkpoint_rows else [], checkpoint_rows)
    write_csv(object_csv, list(object_rows[0].keys()) if object_rows else [], object_rows)

    date_stamp = report_date.isoformat()
    report_path = build_dated_doc_path(ROOT, doc_dir="experiment_results", slug="phase5_surface_rgb", when=report_date)

    baseline_by_scene = {row["scene_id"]: row for row in summary_rows if row["label"] == "baseline"}
    lines = [
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    fmt_int(row["prior_survived_15000"]),
                    fmt_float(row["prior_survival_ratio_15000"], 4),
                ]
            )
            + " |"
        )
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    fmt_int(row["survived_15000"]),
                    fmt_float(row["survival_ratio_15000"], 4),
                ]
            )
            + " |"
        )
    ...
    lines.extend(["", "## Notes", ""])
    for scene_id in SCENES:
        baseline = baseline_by_scene.get(scene_id)
        if baseline is None:
            continue
        lines.append(
            f"- `{scene_id}` baseline absolute final PSNR: {fmt_float(baseline['iter_15000_psnr'], 4)}; "
            f"baseline 3000-iter target PSNR: {fmt_float(baseline['target_psnr'], 4)}"
        )
    lines.append("- 해석은 absolute 값과 함께 scene별 baseline 대비 delta를 우선 본다.")
