...
def main() -> int:
    ...
    output_path = (
        ...
        else ROOT / "docs" / "experiments" / f"replica_gaussian_direct_roomwide_v2_viewcount_sweep_{datetime.now(UTC).strftime('%Y-%m-%d')}.md"
    )
    summary_csv_path = (
        resolve_path(args.summary_csv)
        if args.summary_csv is not None
        else outputs_dir / "reports" / "replica_gaussian_direct_roomwide_v2_viewcount_sweep_summary.csv"
    )
